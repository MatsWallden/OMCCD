...
const int pinPWM=5; //ASSIGNED PIN FOR PWM INPUT
...
//FUNCTIONS
boolean goToPosition(int posNew){ //MAKES THE CARRIER MOVE TO POSITION
  goDirA=posCounter>posNew; //DETERMINES NEW DIRECTION
...
boolean getRange(){
  ...
  while(!digitalRead(pinPosLim)){ 
  goDirection();}//MOVE UNTIL AT THE EDGE OF RANGE 
