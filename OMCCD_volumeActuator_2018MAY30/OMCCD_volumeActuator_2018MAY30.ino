...
double KI=0.05;
double KD=0.01;
double setPoint=300; //TEST AGAINST A DROP OF X µL FIRST
...
void setup() {
  ...
  //Serial.println("WAITING FOR ID REQUEST");
 ...
 // Serial.println("RANGING");
  ...
  /*while(!(Serial.available()>0 && Serial.readString()=="START")){ 
      delay(1);
  }
...
void loop(){
      ...
      //input=Serial.parseFloat();// Serial.parseFloat();//pulseIn(pwm_pin, HIGH); //READ SIGNAL FROM SAMPLE
